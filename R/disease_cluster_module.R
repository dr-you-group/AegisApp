...
diseaseClusterServer <- function(id, data) {
# diseaseClusterServer <- function(id, data, filter = is.numeric) {
...
#   stopifnot(!is.reactive(filter))
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    disease_cluster <- shiny::eventReactive(input$plot, {
      shinyjs::disable("plot")
      ...
      if(data$model() == "spatio-temporal") {
        ...
        plot <- spatioTemporalServer("dzc", list(
          type = shiny::reactive("cluster"),
    ...
    shiny::observeEvent(disease_cluster(), {
      shiny::insertUI(
        session = session,
        selector = "#dzc-output_plots",
        ui = htmltools::tagList(
          lapply(1:length(names(disease_cluster())), function(i) {
            plotname <- paste("dzc-", "disease_cluster_", i, sep="")

            message("render ui plotname: ", plotname)

            leaflet::leafletOutput(plotname)
          }),
          rendered_js_callback_ui(input_id = "dzc-my_input_for_catching_render")
        )
      )
    })
